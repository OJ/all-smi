...
impl TenstorrentExporter {
    ...
    fn export_firmware(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        // ARC firmware
        if let Some(arc_fw) = info.detail.get(keys::ARC_FW_VERSION) {
            let fw_labels = [
        ...
        // Ethernet firmware
        if let Some(eth_fw) = info.detail.get(keys::ETH_FW_VERSION) {
            let fw_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        // Firmware date
        if let Some(fw_date) = info.detail.get(keys::FW_DATE) {
            let fw_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        // DDR firmware
        if let Some(ddr_fw) = info.detail.get(keys::DDR_FW_VERSION) {
            let fw_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        if let Some(spi_fw) = info.detail.get(keys::SPIBOOTROM_FW_VERSION) {
            let fw_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
    ...
    fn export_temperatures(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        if let Some(asic_temp) = info.detail.get(keys::ASIC_TEMPERATURE)
        ...
        if let Some(vreg_temp) = info.detail.get(keys::VREG_TEMPERATURE)
        ...
        // Inlet temperature
        if let Some(inlet_temp) = info.detail.get(keys::INLET_TEMPERATURE)
        ...
        // Outlet temperatures
        if let Some(outlet_temp1) = info.detail.get(keys::OUTLET_TEMPERATURE1)
        ...
        if let Some(outlet_temp2) = info.detail.get(keys::OUTLET_TEMPERATURE2)
    ...
    fn export_clocks(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        // AI clock
        if let Some(aiclk) = info.detail.get(keys::AICLK_MHZ)
        ...
        // AXI clock
        if let Some(axiclk) = info.detail.get(keys::AXICLK_MHZ)
        ...
        // ARC clock
        if let Some(arcclk) = info.detail.get(keys::ARCCLK_MHZ)
    ...
    fn export_power(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        // Voltage
        if let Some(voltage) = info.detail.get(keys::VOLTAGE)
        ...
        // Current
        if let Some(current) = info.detail.get(keys::CURRENT)
        ...
        // Power limits
        if let Some(tdp_limit) = info.detail.get(keys::POWER_LIMIT_TDP)
        ...
        if let Some(tdc_limit) = info.detail.get(keys::POWER_LIMIT_TDC)
        ...
        if let Some(tdp_limit) = info.detail.get(keys::TDP_LIMIT)
            && let Some(power) = CommonNpuExporter::parse_numeric_value(tdp_limit)
        {
            builder
        ...
        if let Some(tdc_limit) = info.detail.get(keys::TDC_LIMIT)
            && let Some(current) = CommonNpuExporter::parse_numeric_value(tdc_limit)
        {
            builder
                .help(
        ...
        // Thermal limit
        if let Some(thermal_limit) = info.detail.get(keys::THERMAL_LIMIT)
        ...
        // Heartbeat
        if let Some(heartbeat) = info.detail.get(keys::HEARTBEAT)
        ...
        if let Some(power_watts) = info.detail.get(keys::POWER_WATTS)
    ...
    fn export_status_health(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        // PCIe status
        if let Some(pcie_status) = info.detail.get(keys::PCIE_STATUS) {
            let status_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        // Ethernet status
        if let Some(eth_status0) = info.detail.get(keys::ETH_STATUS0) {
            let status_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        if let Some(eth_status1) = info.detail.get(keys::ETH_STATUS1) {
            let status_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        if let Some(ddr_status) = info.detail.get(keys::DDR_STATUS)
        ...
        // ARC health counters
        if let Some(arc0_health) = info.detail.get(keys::ARC0_HEALTH)
        ...
        if let Some(arc3_health) = info.detail.get(keys::ARC3_HEALTH)
        ...
        // Faults register
        if let Some(faults) = info.detail.get(keys::FAULTS)
        ...
        // Throttler state
        if let Some(throttler) = info.detail.get(keys::THROTTLER)
        ...
        // Fan metrics
        if let Some(fan_speed) = info.detail.get(keys::FAN_SPEED)
        ...
        if let Some(fan_rpm) = info.detail.get(keys::FAN_RPM)
    ...
    fn export_board_info(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        if let Some(board_type) = info.detail.get(keys::BOARD_TYPE) {
        ...
        // Collection method
        if let Some(method) = info.detail.get(keys::COLLECTION_METHOD) {
            let method_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
    ...
    fn export_pcie_dram(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        // PCIe address
        if let Some(pcie_addr) = info.detail.get(keys::PCIE_ADDRESS) {
            let pcie_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        if let Some(vendor_id) = info.detail.get(keys::PCIE_VENDOR_ID)
            && let Some(device_id) = info.detail.get(keys::PCIE_DEVICE_ID)
        {
            let pcie_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
        ...
        // PCIe generation
        if let Some(pcie_gen) = info.detail.get(keys::PCIE_LINK_GEN)
        ...
        // PCIe width
        if let Some(pcie_width) = info.detail.get(keys::PCIE_LINK_WIDTH)
        ...
        // DRAM speed
        if let Some(dram_speed) = info.detail.get(keys::DRAM_SPEED) {
            let dram_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
