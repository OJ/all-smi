...
impl RebellionsExporter {
    ...
    fn export_firmware_info(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        if let Some(fw_version) = info.detail.get(detail_keys::FIRMWARE_VERSION) {
            let fw_labels = [
        ...
        // KMD version
        if let Some(kmd_version) = info.detail.get(detail_keys::KMD_VERSION) {
            let kmd_labels = [
                ("instance", info.instance.as_str()),
    ...
    fn export_device_info(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        if let Some(_device_name) = info.detail.get(detail_keys::DEVICE_NAME)
            && let Some(sid) = info.detail.get(detail_keys::SERIAL_ID)
        {
    ...
    fn export_performance_state(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        if let Some(pstate) = info.detail.get(detail_keys::PERFORMANCE_STATE) {
            let pstate_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
