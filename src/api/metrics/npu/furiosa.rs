...
impl FuriosaExporter {
    ...
    fn export_device_info(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        if let Some(device_name) = info.detail.get(detail_keys::DEVICE_NAME) {
            let device_labels = [
        ...
        if let Some(chip_name) = info.detail.get(detail_keys::CHIP_NAME) {
            let chip_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
    ...
    fn export_firmware_info(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        if let Some(driver_version) = info.detail.get(detail_keys::DRIVER_VERSION) {
            let driver_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        if let Some(firmware_version) = info.detail.get(detail_keys::FIRMWARE_VERSION) {
            let fw_labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
    ...
    fn export_utilization_metrics(
        ...
        if let Some(util_str) = info.detail.get(detail_keys::UTILIZATION)
        ...
        if let Some(compute_util_str) = info.detail.get(detail_keys::COMPUTE_UTILIZATION)
    ...
    fn export_memory_metrics(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        if let Some(mem_used_str) = info.detail.get(detail_keys::MEMORY_USED)
        ...
        if let Some(mem_total_str) = info.detail.get(detail_keys::MEMORY_TOTAL)
    ...
    fn export_clock_metrics(&self, builder: &mut MetricBuilder, info: &GpuInfo, index: usize) {
        ...
        if let Some(clock_str) = info.detail.get(detail_keys::CLOCK_MHZ)
        ...
        if let Some(mem_clock_str) = info.detail.get(detail_keys::MEMORY_CLOCK_MHZ)
