...
impl NpuExporter for GoogleTpuExporter {
    ...
    fn export_vendor_metrics(
        ...
        if let Some(chip_version) = info.detail.get(keys::CHIP_VERSION) {
            let labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        if let Some(accel_type) = info.detail.get(keys::ACCELERATOR_TYPE) {
            let labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        // 2. Core Counts
        if let Some(core_count) = info.detail.get(keys::CORE_COUNT)
        ...
        if let Some(tc_count) = info.detail.get(keys::TENSORCORE_COUNT)
        ...
        // 3. Memory Type
        if let Some(mem_type) = info.detail.get(keys::MEMORY_TYPE) {
            let labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        if let Some(lib_ver) = info.detail.get(keys::LIB_VERSION) {
            let labels = [
                ("npu", info.name.as_str()),
                ("instance", info.instance.as_str()),
                ("npu_uuid", info.uuid.as_str()),
        ...
        // 5. Max Power Limit
        if let Some(max_power_str) = info.detail.get(keys::MAX_POWER) {
        ...
        if let Some(q_size_str) = info.detail.get(keys::HLO_QUEUE_SIZE)
