...
impl MetricPresenceFlags {
    ...
    fn from_chassis_info(chassis_info: &[ChassisInfo]) -> Self {
        ...
        for chassis in chassis_info {
            ...
            flags.has_cpu_power |= chassis.detail.contains_key(detail_keys::CPU_POWER_WATTS);
            flags.has_gpu_power |= chassis.detail.contains_key(detail_keys::GPU_POWER_WATTS);
            flags.has_ane_power |= chassis.detail.contains_key(detail_keys::ANE_POWER_WATTS);
    ...
    fn export_metrics(&self) -> String {
        ...
        // Export chassis info metric with DMI/platform details as labels
        {
            ...
            let promoted: &[(&str, &str)] = &[
                (detail_keys::PRODUCT_NAME, "product_name"),
                (detail_keys::VENDOR, "vendor"),
                (detail_keys::BOARD, "board"),
                (detail_keys::VERSION, "version"),
                (detail_keys::BIOS_VERSION, "bios_version"),
                (detail_keys::PLATFORM, "platform"),
            ];
        ...
        if flags.has_cpu_power {
            ...
            for chassis in self.chassis_info {
                if let Some(power_str) = chassis.detail.get(detail_keys::CPU_POWER_WATTS)
        ...
        if flags.has_gpu_power {
            ...
            for chassis in self.chassis_info {
                if let Some(power_str) = chassis.detail.get(detail_keys::GPU_POWER_WATTS)
                    && let Ok(power) = power_str.parse::<f64>()
                {
                    builder.metric(
                        "all_smi_chassis_gpu_power_watts",
        ...
        if flags.has_ane_power {
            ...
            for chassis in self.chassis_info {
                if let Some(power_str) = chassis.detail.get(detail_keys::ANE_POWER_WATTS)
                    && let Ok(power) = power_str.parse::<f64>()
                {
                    builder.metric(
                        "all_smi_chassis_ane_power_watts",
...
mod tests {
    ...
    fn test_chassis_info_dmi_labels_metric() {
        ...
        detail.insert(
            detail_keys::PRODUCT_NAME.to_string(),
            "DGX H100".to_string(),
        );
        detail.insert(detail_keys::VENDOR.to_string(), "NVIDIA".to_string());
        detail.insert(detail_keys::BOARD.to_string(), "H100-BOARD".to_string());
        detail.insert(detail_keys::BIOS_VERSION.to_string(), "1.0.0".to_string());
        detail.insert(detail_keys::PLATFORM.to_string(), "Linux".to_string());
    ...
    fn test_metric_presence_flags_all_present() {
        let mut detail = std::collections::HashMap::new();
        detail.insert(detail_keys::CPU_POWER_WATTS.to_string(), "15.0".to_string());
        detail.insert(
            detail_keys::GPU_POWER_WATTS.to_string(),
            "200.0".to_string(),
        );
        detail.insert(detail_keys::ANE_POWER_WATTS.to_string(), "5.0".to_string());
