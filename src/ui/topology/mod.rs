...
pub struct TopologyGpu {
    /// Ordinal index inside the host, derived from the `detail[detail_keys::INDEX]`
...
mod tests {
    ...
    fn pcie_formatting_accepts_the_bare_width_spelling() {
        let mut gpu = mk_gpu(0, Some(0), vec![]);
        gpu.detail
            .insert(detail_keys::PCIE_GENERATION.to_string(), "5".to_string());
        gpu.detail
            .insert(detail_keys::PCIE_LINK_WIDTH.to_string(), "16".to_string());
        let model = TopologyModel::from_host("h", &[gpu]);
    ...
    fn pcie_formatting_accepts_the_prefixed_width_spelling() {
        ...
        gpu.detail
            .insert(detail_keys::PCIE_GENERATION.to_string(), "5".to_string());
        gpu.detail
            .insert(detail_keys::PCIE_LINK_WIDTH.to_string(), "x16".to_string());
        let model = TopologyModel::from_host("h", &[gpu]);
        assert_eq!(model.gpus[0].pcie_display, "Gen5 x16");
    }
