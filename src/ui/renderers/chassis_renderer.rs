...
pub fn print_chassis_info<W: Write>(
    ...
    let has_power_breakdown = info.detail.contains_key(detail_keys::CPU_POWER_WATTS)
        || info.detail.contains_key(detail_keys::GPU_POWER_WATTS)
        || info.detail.contains_key(detail_keys::ANE_POWER_WATTS);

    if has_power_breakdown {
        print_colored_text(stdout, " │", Color::DarkGrey, None, None);

        if let Some(cpu_power) = info.detail.get(detail_keys::CPU_POWER_WATTS)
        ...
        if let Some(gpu_power) = info.detail.get(detail_keys::GPU_POWER_WATTS)
        ...
        if let Some(ane_power) = info.detail.get(detail_keys::ANE_POWER_WATTS)
    ...
    if let Some(power) = info.total_power_watts {
        ...
        let is_apple_silicon =
            info.detail.get(detail_keys::PLATFORM) == Some(&"Apple Silicon".to_string());
