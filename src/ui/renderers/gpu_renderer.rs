...
pub fn print_gpu_info<W: Write>(
    ...
    let vram_display =
        if info.detail.get(detail_keys::METRICS_AVAILABLE) == Some(&"false".to_string()) {
            format!("{:>11}", "N/A")
        } else {
    ...
    let power_display = match info.power_consumption_reading() {
        ...
        Some(power) => match info
            .detail
            .get(detail_keys::POWER_LIMIT_MAX)
    ...
    if info.device_type == "TPU" {
        let hlo_queue_size = info
            .detail
            .get(detail_keys::HLO_QUEUE_SIZE)
            .map(|s| s.as_str())
            .unwrap_or("0");
    ...
    if let Some(lib_name) = info.detail.get(detail_keys::LIB_NAME) {
        if let Some(lib_version) = info.detail.get(detail_keys::LIB_VERSION) {
        ...
        if let Some(cuda_version) = info.detail.get(detail_keys::CUDA_VERSION) {
            print_colored_text(stdout, " CUDA:", Color::Green, None, None);
            print_colored_text(stdout, cuda_version, Color::White, None, None);
        } else if let Some(rocm_version) = info.detail.get(detail_keys::ROCM_VERSION) {
