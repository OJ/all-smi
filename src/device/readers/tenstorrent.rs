...
fn extract_static_info(chip: &Chip) -> Option<(DeviceStaticInfo, TenstorrentStaticInfo)> {
    ...
    let mut builder = DetailBuilder::new()
        .insert(detail_keys::BOARD_TYPE, board_type)
        .insert(detail_keys::BOARD_ID, telem.board_serial_number_hex())
        .insert(detail_keys::ARC_FW_VERSION, telem.arc_fw_version())
        .insert(detail_keys::ETH_FW_VERSION, telem.eth_fw_version())
        .insert(detail_keys::FW_DATE, telem.firmware_date());
    ...
    if let Ok(Some(device_info)) = chip.get_device_info() {
        ...
        builder = builder
            .insert(detail_keys::PCIE_ADDRESS, &pcie_address)
            .insert(
                detail_keys::PCIE_VENDOR_ID,
                format!("0x{:04x}", device_info.vendor),
            )
            .insert(
                detail_keys::PCIE_DEVICE_ID,
    ...
    builder = builder.insert_optional(detail_keys::DDR_FW_VERSION, ddr_fw_version);
    ...
    builder = builder.insert_optional(detail_keys::SPIBOOTROM_FW_VERSION, spibootrom_fw_version);
...
fn build_device_details(
    ...
    // Dynamic telemetry
    detail.insert(
        detail_keys::VDD_VOLTAGE.to_string(),
        format!("{:.3}V", telem.voltage()),
    );
    detail.insert(
        detail_keys::CURRENT.to_string(),
        format!("{:.2}A", telem.current()),
    );
    detail.insert(
        detail_keys::ASIC_TEMPERATURE.to_string(),
        format!("{:.1}°C", telem.asic_temperature()),
    );
    detail.insert(
        detail_keys::VR_TEMPERATURE.to_string(),
        format!("{:.1}°C", telem.vreg_temperature()),
    );

    if telem.board_temperature != 0 {
        detail.insert(
            detail_keys::INLET_TEMPERATURE.to_string(),
            format!("{:.1}°C", telem.inlet_temperature()),
        );
    }

    detail.insert(
        detail_keys::AI_CLOCK.to_string(),
        format!("{}MHz", telem.ai_clk()),
    );
    detail.insert(
        detail_keys::ARC_CLOCK.to_string(),
        format!("{}MHz", telem.arc_clk()),
    );
    detail.insert(
        detail_keys::AXI_CLOCK.to_string(),
        format!("{}MHz", telem.axi_clk()),
    );

    // Add unified AI acceleration library labels if not already present
    detail
        .entry(detail_keys::LIB_NAME.to_string())
        .or_insert("Luwen".to_string());
    if let Some(arc_fw) = detail.get(detail_keys::ARC_FW_VERSION) {
        detail.insert(detail_keys::LIB_VERSION.to_string(), arc_fw.clone());
    }
