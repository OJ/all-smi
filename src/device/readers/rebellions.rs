        ...
        self.device_static_info.get_or_init(|| {
            ...
            for device in devices_to_process {
                ...
                let detail = DetailBuilder::new()
                    .insert(detail_keys::SERIAL_ID, &device.sid)
                    .insert(detail_keys::FIRMWARE_VERSION, &device.fw_ver)
                    .insert(detail_keys::DEVICE_PATH, &device.device)
                    .insert(detail_keys::BOARD_INFO, &device.board_info)
                    .insert_pci_info(
                        Some(&device.pci.bus_id),
                        None, // Rebellions doesn't provide PCIe generation separately
                        Some(&device.pci.link_width),
                    )
                    .insert(detail_keys::PCI_LINK_SPEED, &device.pci.link_speed)
                    .insert(detail_keys::PCI_NUMA_NODE, &device.pci.numa_node)
                    .build();
...
fn create_gpu_info_from_device(
    ...
    detail.insert(
        detail_keys::KMD_VERSION.to_string(),
        kmd_version.to_string(),
    );

    // Dynamic values
    detail.insert(detail_keys::STATUS.to_string(), device.status.clone());
    detail.insert(
        detail_keys::PERFORMANCE_STATE.to_string(),
        device.pstate.clone(),
    );

    // Add unified AI acceleration library labels
    detail.insert(detail_keys::LIB_NAME.to_string(), "RBLN-SDK".to_string());
    detail.insert(
