...
fn get_gpu_info_populates_basic_fields() {
    ...
    assert_eq!(
        g.detail.get(keys::VARIANT).map(String::as_str),
        Some("Discrete")
    );
    assert_eq!(g.detail.get(keys::DRIVER).map(String::as_str), Some("i915"));
    // No engine sysfs entries in this fixture -> reader must surface
    // the explanatory note, NOT the obsolete `intel_gpu_top` text.
    assert_eq!(
        g.detail.get(keys::UTILIZATION).map(String::as_str),
    ...
    assert_eq!(
        g.detail.get(keys::ARCHITECTURE).map(String::as_str),
        Some("Alchemist (Xe-HPG, A-series)")
    );
    assert_eq!(
        g.detail.get(keys::SYCL_CAPABLE).map(String::as_str),
        Some("Yes")
    );
    ...
    assert_eq!(
        g.detail.get(keys::METRICS_SOURCE).map(String::as_str),
...
fn get_gpu_info_publishes_fan_speed_as_field_and_detail() {
    ...
    assert_eq!(
        info[0].detail.get(keys::FAN_SPEED).map(String::as_str),
        Some("1730 RPM")
    );
    assert_eq!(
        info[0].detail.get(keys::SOURCE_FAN).map(String::as_str),
        Some("hwmon")
    );
}
...
fn get_gpu_info_leaves_fan_speed_unset_without_a_tachometer() {
    ...
    assert!(!info[0].detail.contains_key(keys::FAN_SPEED));
    assert_eq!(
        info[0].detail.get(keys::SOURCE_FAN).map(String::as_str),
        Some("unavailable")
    );
...
fn get_gpu_info_clamps_a_garbled_fan_reading_before_either_write() {
    // A corrupted `fan1_input` file parses as a valid, but implausible,
    // u32. That must never reach `GpuInfo::fan_speed_rpm` or the `Fan
    // Speed` detail string unclamped, and the two must keep agreeing with
    // each other after the clamp the same way they do for a normal
    // reading (see `get_gpu_info_publishes_fan_speed_as_field_and_detail`).
    let dir = tempdir().unwrap();
    let root = dir.path();
    let card = make_card(root, 0, "0x8086", "i915", "0x56A0");
    let hwmon = card.join("device").join("hwmon").join("hwmon0");
...
fn get_gpu_info_integrated_reports_zero_memory() {
    ...
    assert_eq!(
        info[0].detail.get(keys::VARIANT).map(String::as_str),
        Some("Integrated")
    );
    assert!(
        info[0].detail.contains_key(keys::MEMORY),
        "integrated GPUs should explain the shared-memory situation"
    );
    // Meteor Lake / Core Ultra iGPU is Xe-LPG and SYCL-capable.
    assert_eq!(
        info[0].detail.get(keys::ARCHITECTURE).map(String::as_str),
        Some("Xe-LPG (Meteor Lake)")
    );
    assert_eq!(
        info[0].detail.get(keys::SYCL_CAPABLE).map(String::as_str),
        Some("Yes")
    );
}
...
fn get_gpu_info_second_call_surfaces_engine_percent() {
    ...
    assert!(
        g.detail.contains_key(&keys::engine("render")),
    ...
    // available.
    assert!(
        !g.detail.contains_key(keys::UTILIZATION),
        "Utilization note should be cleared when engine data is live, got: {:?}",
        g.detail.get(keys::UTILIZATION)
    );
}
