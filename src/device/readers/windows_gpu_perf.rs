...
pub fn note_metrics_source(detail: &mut HashMap<String, String>, source: &str) {
    let entry = detail
        .entry(detail_keys::METRICS_SOURCE.to_string())
        .or_default();
    if entry.is_empty() {
...
pub fn apply_to_gpu_info(gpu: &mut GpuInfo, metrics: &AdapterMetrics) {
    ...
    if let Some(budget) = metrics.process_budget {
        gpu.detail.insert(
            detail_keys::VRAM_BUDGET_PROCESS.to_string(),
            format!("{budget} bytes"),
        );
        touched_dxgi = true;
    }
    if let Some(usage) = metrics.process_current_usage {
        gpu.detail.insert(
            detail_keys::VRAM_USAGE_PROCESS.to_string(),
            format!("{usage} bytes"),
        );
        touched_dxgi = true;
    }
    ...
    if let Some(used) = metrics.used_memory {
        ...
        gpu.detail.insert(
            detail_keys::SOURCE_MEMORY_USED.to_string(),
            if metrics.memory_is_shared {
...
mod tests {
    ...
    fn blank_gpu() -> GpuInfo {
        ...
        detail.insert(detail_keys::METRICS_SOURCE.to_string(), "WMI".to_string());
        detail.insert(
            detail_keys::SOURCE_UTILIZATION.to_string(),
            "unavailable".to_string(),
        );
        detail.insert(detail_keys::SOURCE_MEMORY.to_string(), "WMI".to_string());
        GpuInfo {
    ...
    fn dxgi_total_replaces_the_truncated_wmi_value() {
        ...
        assert_eq!(gpu.detail[detail_keys::SOURCE_MEMORY], "DXGI");
        assert_eq!(gpu.detail[detail_keys::METRICS_SOURCE], "WMI + DXGI");
    }
    ...
    fn pdh_fields_are_recorded_with_their_source() {
        ...
        assert_eq!(gpu.detail[detail_keys::SOURCE_MEMORY_USED], "PDH");
    ...
    fn absent_fields_leave_the_baseline_untouched() {
        ...
        assert_eq!(gpu.detail[detail_keys::SOURCE_UTILIZATION], "unavailable");
        assert_eq!(gpu.detail[detail_keys::METRICS_SOURCE], "WMI + DXGI");
    }
    ...
    fn metrics_source_starts_clean_when_absent() {
        ...
        assert_eq!(detail[detail_keys::METRICS_SOURCE], "DXGI");
        note_metrics_source(&mut detail, "PDH");
        assert_eq!(detail[detail_keys::METRICS_SOURCE], "DXGI + PDH");
    }
    ...
    fn pairing_an_empty_snapshot_changes_nothing() {
        ...
        assert_eq!(gpus[0].detail[detail_keys::METRICS_SOURCE], "WMI");
    }
    ...
    fn unmatched_gpus_keep_the_wmi_baseline() {
        ...
        assert_eq!(gpus[1].detail[detail_keys::METRICS_SOURCE], "WMI");
    ...
    fn process_scoped_dxgi_figures_are_labelled_and_kept_out_of_used_memory() {
        ...
        assert_eq!(
            gpu.detail[detail_keys::VRAM_BUDGET_PROCESS],
            "7000000000 bytes"
        );
        assert_eq!(gpu.detail[detail_keys::VRAM_USAGE_PROCESS], "123456 bytes");
        assert!(!gpu.detail.contains_key(detail_keys::SOURCE_MEMORY_USED));
    }
