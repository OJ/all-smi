...
impl NvidiaJetsonGpuReader {
    ...
    fn get_static_info(&self) -> &DeviceStaticInfo {
        self.static_info.get_or_init(|| {
                    ...
                    if line.contains("CUDA Version:") {
                        if let Some(version_part) = line.split("CUDA Version:").nth(1) {
                            let version = version_part
                                ...
                                .to_string();
                            builder = builder
                                .insert(detail_keys::CUDA_VERSION, &version)
            ...
            builder = builder.insert_optional(detail_keys::JETPACK_VERSION, jetpack_version);
            ...
            {
                detail.insert(detail_keys::L4T_VERSION.to_string(), version.to_string());
            }

            // Static hardware info
            detail.insert(detail_keys::GPU_TYPE.to_string(), "Integrated".to_string());
            detail.insert(detail_keys::ARCHITECTURE.to_string(), "Tegra".to_string());
