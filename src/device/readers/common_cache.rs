...
impl DetailBuilder {
    ...
    pub fn insert_lib_info(self, name: &str, version: Option<&str>) -> Self {
        let key = keys::library_version(name);
    ...
    pub fn insert_pci_info(
        ...
        self.insert_optional(keys::PCI_BUS_ID, bus_id)
            .insert_optional(keys::PCIE_GENERATION, link_gen)
            .insert_optional(keys::PCIE_LINK_WIDTH, link_width)
    }
...
///     keys::NAME => Some(device_name),
///     keys::UUID => device_uuid,  // Option<String>
///     keys::DRIVER => Some("nvidia-smi".to_string()),
///     keys::TEMPERATURE => temp.map(|t| format!("{}°C", t)),
/// };
/// ```
#[macro_export]
...
mod tests {
    ...
    fn test_detail_builder() {
        ...
        assert_eq!(details.get(keys::CUDA_VERSION), Some(&"12.0".to_string()));
        assert_eq!(details.get(keys::PCI_BUS_ID), Some(&"00:03.0".to_string()));
        assert_eq!(details.get(keys::PCIE_GENERATION), Some(&"4".to_string()));
        assert_eq!(details.get(keys::PCIE_LINK_WIDTH), Some(&"x16".to_string()));
    }
    ...
    fn test_build_detail_map_macro() {
        ...
        let details = build_detail_map! {
            keys::NAME => name,
            keys::UUID => uuid,
            keys::TEMPERATURE => temp.map(|t| format!("{t}°C")),
        };

        assert_eq!(details.get(keys::NAME), Some(&"Device".to_string()));
        assert_eq!(details.get(keys::UUID), None);
        assert_eq!(details.get(keys::TEMPERATURE), Some(&"75°C".to_string()));
    }
}
