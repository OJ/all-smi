        ...
        self.device_static_info.get_or_init(|| {
            ...
            for device in devices_to_process {
                ...
                let detail = DetailBuilder::new()
                    .insert(detail_keys::DEVICE_INDEX, device.index.to_string())
                    .insert(detail_keys::CHIP_VERSION, &device.chip_version)
                    .insert(detail_keys::ACCELERATOR_TYPE, &device.accelerator_type)
                    .insert(detail_keys::CORE_COUNT, device.core_count.to_string())
                    .insert(
                        detail_keys::TENSORCORE_COUNT,
                        generation.tensor_cores().to_string(),
                    )
                    .insert(detail_keys::MEMORY_TYPE, generation.memory_type())
                    .insert(
                        detail_keys::TOTAL_MEMORY,
                        format_memory_size(generation.hbm_size_bytes()),
                    )
                    .insert(detail_keys::MAX_POWER, format!("{:.0} W", device.power_max))
                    .insert_optional(
                        detail_keys::TPU_RUNTIME_VERSION,
                    ...
                    .insert(detail_keys::LIB_NAME, "libtpu")
                    .insert_optional(
                        detail_keys::LIB_VERSION,
                        if device.tpu_runtime_version.is_empty() {
                            None
                        } else {
                            Some(&device.tpu_runtime_version)
...
fn create_gpu_info_from_device(
    ...
    detail.insert(
        detail_keys::CURRENT_POWER.to_string(),
        format!("{:.1} W", device.power_draw),
    );
    detail.insert(
        detail_keys::USED_MEMORY.to_string(),
...
mod tests {
    ...
    fn test_create_gpu_info_from_mock_device() {
        ...
        // Check detail fields
        assert_eq!(
            info.detail.get(detail_keys::LIB_NAME),
            Some(&"libtpu".to_string())
        );
        assert_eq!(
            info.detail.get(detail_keys::LIB_VERSION),
            Some(&"2.13.0".to_string())
        );

        // Check HLO metrics in detail
        assert_eq!(
            info.detail.get(detail_keys::HLO_QUEUE_SIZE),
            Some(&"3".to_string())
        );
        assert_eq!(
            info.detail.get(detail_keys::HLO_EXEC_MEAN),
            Some(&"125.5 µs".to_string())
        );
        assert_eq!(
            info.detail.get(detail_keys::HLO_EXEC_P50),
