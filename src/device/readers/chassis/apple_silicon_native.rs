...
impl ChassisReader for AppleSiliconNativeChassisReader {
    fn get_chassis_info(&self) -> Option<ChassisInfo> {
        ...
        detail.insert(keys::PLATFORM.to_string(), "Apple Silicon".to_string());
        ...
        detail.insert(
            keys::CPU_POWER_WATTS.to_string(),
            format!("{cpu_power_watts:.2}"),
        );
        detail.insert(
            keys::GPU_POWER_WATTS.to_string(),
            format!("{gpu_power_watts:.2}"),
        );
        detail.insert(
            keys::ANE_POWER_WATTS.to_string(),
        ...
        if data.e_cluster_frequency > 0 {
            detail.insert(
                keys::E_CLUSTER_FREQ_MHZ.to_string(),
                data.e_cluster_frequency.to_string(),
            );
        }
        if data.p_cluster_frequency > 0 {
            detail.insert(
                keys::P_CLUSTER_FREQ_MHZ.to_string(),
