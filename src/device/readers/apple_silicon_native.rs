...
impl AppleSiliconNativeGpuReader {
    ...
    fn ensure_initialized(&self) {
        ...
        let mut builder = DetailBuilder::new()
            .insert(keys::GPU_TYPE, "Integrated")
            .insert_optional(keys::DRIVER_VERSION, driver_version.as_ref());

        if let Some(count) = gpu_core_count {
            builder = builder.insert(keys::GPU_CORE_COUNT, count.to_string());
        }
...
fn build_gpu_info(
    ...
    detail.insert(keys::API.to_string(), "Native (IOReport/SMC)".to_string());
    ...
    if let Some(cpu_t) = cpu_temp {
        detail.insert(keys::CPU_TEMPERATURE.to_string(), format!("{cpu_t:.1}"));
    }
    if let Some(gpu_t) = gpu_temp {
        detail.insert(keys::GPU_TEMPERATURE.to_string(), format!("{gpu_t:.1}"));
    }

    // Add unified AI acceleration library labels
    detail.insert(keys::LIB_NAME.to_string(), "Metal".to_string());
    if let Some(driver_ver) = static_info.detail.get(keys::DRIVER_VERSION)
        && driver_ver != "Unknown"
    {
        let lib_ver = driver_ver
            ...
            .to_string();
        detail.insert(keys::LIB_VERSION.to_string(), lib_ver);
    }
...
mod tests {
    ...
    fn static_info() -> DeviceStaticInfo {
        DeviceStaticInfo::with_details(
            ...
            DetailBuilder::new()
                .insert(keys::GPU_TYPE, "Integrated")
                .insert(keys::DRIVER_VERSION, "Metal 3")
                .build(),
        )
    }
    ...
    fn degraded_path_keeps_identity_and_memory() {
        ...
        assert_eq!(
            info.detail.get(keys::NATIVE_METRICS).map(String::as_str),
            Some("unavailable"),
        ...
        assert!(!info.detail.contains_key(keys::COMBINED_POWER_MW));
        assert!(!info.detail.contains_key(keys::THERMAL_PRESSURE));
        assert!(!info.detail.contains_key(keys::CPU_TEMPERATURE));
    }
