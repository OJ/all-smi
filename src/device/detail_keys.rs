    ...
    fn no_reader_or_consumer_spells_a_key_by_hand() {
        ...
        while let Some(dir) = stack.pop() {
            for entry in std::fs::read_dir(&dir).expect("read src/") {
                ...
                if path.extension().is_none_or(|e| e != "rs") || path.ends_with("detail_keys.rs") {
                    continue;
                }
        ...
        assert!(
            offenders.is_empty(),
            "detail keys spelled as literals instead of `detail_keys` constants:\n  {}\n\n\
             Add a constant to `device::detail_keys` and use it. If the literal is \
