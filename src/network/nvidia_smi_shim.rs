...
fn parse_row(
    ...
    if !driver_version.is_empty() {
        detail.insert(detail_keys::DRIVER_VERSION.to_string(), driver_version);
    }
    detail.insert(
        detail_keys::TRANSPORT.to_string(),
        "ssh/nvidia-smi".to_string(),
    );
...
";
    ...
    fn parses_two_gpu_golden_sample() {
        ...
        assert_eq!(
            g0.detail.get(detail_keys::DRIVER_VERSION),
            Some(&"550.54.15".to_string())
        );
        assert_eq!(
            g0.detail.get(detail_keys::TRANSPORT),
