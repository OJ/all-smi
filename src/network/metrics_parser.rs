...
impl MetricsParser {
    ...
    fn process_gpu_metrics(
        ...
        let gpu_index = labels
            ...
            .or_else(|| labels.get(detail_keys::INDEX))
            .cloned()
            .unwrap_or_default();
        ...
        let gpu_info = gpu_info_map.entry(gpu_uuid.clone()).or_insert_with(|| {
            ...
            detail.insert(detail_keys::INDEX.to_string(), gpu_index.clone());
            GpuInfo {
                uuid: gpu_uuid.clone(),
        ...
        match metric_name {
            "gpu_power_limit_max_watts" => {
                gpu_info
                    .detail
                    .insert(detail_keys::POWER_LIMIT_MAX.to_string(), value.to_string());
            }
            "gpu_info" => {
                // Extract device type
    ...
    fn process_cpu_metrics(
        ...
        match metric_name {
            ...
            "cpu_info" => {
                ...
                if let Some(architecture) = labels.get(detail_keys::ARCHITECTURE) {
    ...
    fn update_instance_names(
        ...
        for gpu_info in gpu_info_map.values_mut() {
            gpu_info.detail.insert(
                detail_keys::INSTANCE_NAME.to_string(),
    ...
    fn process(
        ...
        match metric_name {
            ...
            "vgpu_scheduler_state" => {
                ...
                if let Some(flag) = labels.get(detail_keys::ARR_SUPPORTED) {
...
mod tests {
    ...
    fn test_parse_labels() {
        ...
        assert_eq!(labels.get(detail_keys::INDEX).unwrap(), "0");
...
all_smi_npu_firmware_info{npu=\"Tenstorrent Wormhole n150s\", instance=\"node-7\", npu_uuid=\"NPU-A\", npu_index=\"0\", firmware=\"1.2.3\"} 1\n";
        ...
        assert_eq!(
            npu.detail.get(detail_keys::INDEX).map(String::as_str),
            Some("0")
        );
        assert_eq!(
            npu.detail.get(detail_keys::FIRMWARE).map(String::as_str),
            Some("1.2.3")
        );
    }
