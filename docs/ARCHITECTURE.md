...
- Per-process GPU memory accounting via `src/device/readers/intel_gpu_fdinfo.rs`: walks `/proc/<pid>/fd/` to find file descriptors opened against an Intel DRM minor (`cardN` or `renderD<M>`), parses the matching `/proc/<pid>/fdinfo/<fd>` block for `drm-resident-{local0,system}` (i915) or `drm-resident-{vram0,gtt}` (xe), and dedupes shared fds by `drm-client-id` so processes that `dup(2)` a DRM fd are not double-counted. Permission errors degrade silently per-process. Per-process engine-time utilization (delta-tracked counters keyed on `drm-client-id`) is deferred to a follow-up.
- Level Zero Sysman backend via `src/device/readers/intel_gpu_level_zero.rs`, gated on the `all_smi_level_zero` cfg that `build.rs` emits for every Linux and Windows target. It adds no dependency and opens nothing on a machine with no Intel GPU, so it is unconditional rather than opt-in; the `level_zero` cargo feature survives as an accepted no-op. It dynamically loads `libze_loader.so.1` / `libze_loader.so` on Linux or `ze_loader.dll` on Windows via `libloading`, prefers `zesInit` when present, never mutates `ZES_ENABLE_SYSMAN` from lazy reader initialization, and enumerates Intel devices by PCI BDF. Sysman is the preferred Intel vendor source per metric family when values are fresh and sane: temperature, fresh energy-counter power deltas, dedicated/local memory state, fresh engine-activity deltas, and frequency can override the sysfs/WMI baseline. Seeded delta families (`power`, `engine util`) are tracked as seeded and do not overwrite fallbacks until a second sample produces a valid delta. Linux fan keeps hwmon priority and falls back to Sysman; Windows uses Sysman fan data when available. Missing loaders, missing optional symbols, unsupported domains, permission errors, and bad return codes degrade that metric family only. `detail["Source: <field>"]` records mixed-source results, while `detail["Metrics Source"]` remains a coarse summary.
...
# Accepted no-op. The Intel Level Zero (Sysman) backend it used to gate
# is compiled into every Linux and Windows build through the
# `all_smi_level_zero` cfg that build.rs emits.
level_zero = []
```
